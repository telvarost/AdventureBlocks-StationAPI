...
public class BlueBarrier extends TemplateBlock {

    public BlueBarrier(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_BARRIERS_CONFIG.DISABLE_ALL_BARRIER_RENDERING
            && Config.config.RENDER_BARRIERS_CONFIG.BLUE_BARRIER
        ) {
            return true;
        } else {
            return false;
    ...
    public boolean isSolidFace(BlockView blockView, int x, int y, int z, int i) {
        if (ModHelper.ModHelperFields.isBlueBarrierActive) {
