...
public class OnEffect extends TemplateBlock {

    public OnEffect(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_EFFECTS_CONFIG.DISABLE_ALL_EFFECT_RENDERING
            && Config.config.RENDER_EFFECTS_CONFIG.ON_EFFECT
        ) {
            return true;
        } else {
            return false;
    ...
    public void onEntityCollision(World world, int x, int y, int z, Entity entity) {
        if (null != entity && !(entity instanceof Particle)) {
            ...
            if (ModHelper.ModHelperFields.eventCounter != lastEventState) {
                ...
                /** - Activate flames for blocks in 5x5 cube */
                byte var5 = 2;
                for(int var6 = x - var5; var6 <= x + var5; ++var6) {
                    for(int var7 = y - var5; var7 <= y + var5; ++var7) {
                        for(int var8 = z - var5; var8 <= z + var5; ++var8) {
                            if (  world.getBlockId(var6, var7, var8) == Block.LIT_REDSTONE_TORCH.id
                               || world.getBlockId(var6, var7, var8) == Block.REDSTONE_TORCH.id
                            ) {
                                int meta = world.getBlockMeta(var6, var7, var8);
                                world.setBlock(var6, var7, var8, Block.TORCH.id, meta);
                                world.setBlockMeta(var6, var7, var8, meta);
                            }

                            if (world.getBlockId(var6, var7, var8) == Block.NETHERRACK.id) {
                                if (world.getBlockId(var6, var7 + 1, var8) == 0) {
                                    world.setBlock(var6, var7 + 1, var8, Block.FIRE.id);
                                }
                            }

                            if (world.getBlockId(var6, var7, var8) == Block.FURNACE.id) {
                                FurnaceBlock.updateLitState(true, world, var6, var7, var8);
                            }
                        }
                    }
                }
