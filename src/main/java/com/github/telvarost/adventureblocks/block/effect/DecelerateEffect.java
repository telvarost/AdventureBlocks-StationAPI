...
public class DecelerateEffect extends TemplateBlock {

    public DecelerateEffect(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_EFFECTS_CONFIG.DISABLE_ALL_EFFECT_RENDERING
            && Config.config.RENDER_EFFECTS_CONFIG.DECELERATE_EFFECT
        ) {
            return true;
        } else {
            return false;
    ...
    public void onEntityCollision(World world, int x, int y, int z, Entity entity) {
        if (null != entity && !(entity instanceof Particle)) {
            /** - Decelerate Entity */
            entity.velocityX /= 1.21;
            entity.velocityY /= 1.21;
            entity.velocityZ /= 1.21;
        }
    }
}