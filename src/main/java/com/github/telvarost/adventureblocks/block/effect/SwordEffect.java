...
public class SwordEffect extends TemplateBlock {

    public SwordEffect(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_EFFECTS_CONFIG.DISABLE_ALL_EFFECT_RENDERING
            && Config.config.RENDER_EFFECTS_CONFIG.SWORD_EFFECT
        ) {
            return true;
        } else {
            return false;
    ...
    public void onEntityCollision(World world, int x, int y, int z, Entity entity) {
        if (null != entity && entity instanceof PlayerEntity) {
            ...
            if (ModHelper.ModHelperFields.eventCounter != lastEventState) {
                ...
                /** - Equip sword if none is equipped */
                PlayerInventory currentInventory = ((PlayerEntity) entity).inventory;
                if (null == currentInventory.getSelectedItem()) {
                    currentInventory.setStack(currentInventory.selectedSlot , new ItemStack(Item.GOLDEN_SWORD, 1));
                }
            }
        }
    }
