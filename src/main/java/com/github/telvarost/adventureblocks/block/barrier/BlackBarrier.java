...
public class BlackBarrier extends TemplateBlock {

    public BlackBarrier(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_BARRIERS_CONFIG.DISABLE_ALL_BARRIER_RENDERING
            && Config.config.RENDER_BARRIERS_CONFIG.BLACK_BARRIER
        ) {
            return true;
        } else {
            return false;
