...
public class PinkBarrier extends TemplateBlock {

    public PinkBarrier(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_BARRIERS_CONFIG.DISABLE_ALL_BARRIER_RENDERING
            && Config.config.RENDER_BARRIERS_CONFIG.PINK_BARRIER
        ) {
            return true;
        } else {
            return false;
    ...
    public void addIntersectingBoundingBox(World world, int x, int y, int z, Box box, ArrayList boxes) {
        ...
        if (var7 != null && box.intersects(var7)) {
            if (  (ModHelper.ModHelperFields.collisionEntity instanceof AnimalEntity)
               || (ModHelper.ModHelperFields.collisionEntity instanceof SquidEntity)
            ) {
                boxes.add(var7);
            }
        }
